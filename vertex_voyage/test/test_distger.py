...
import numpy as np
from vertex_voyage import distger
...
class TestDistGER(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.node2vec = DistGER(dim=2, min_walk_size=5, max_walk_size=10, n_walks=10, window_size=2, epochs=1, seed=42, use_threads=False)
    ...
    def test_random_walks(self):
        ...
        for walk in walks:
            self.assertLessEqual(len(walk), self.node2vec.walk_size)
        ...
        for walk in walks:
            self.assertLessEqual(len(walk), node2vec.walk_size)
