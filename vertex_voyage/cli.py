...
class CustomCLICommandExecutor:
    pass 
...
def _enumerated_event_stream(dataset_name: str, enumerator):
    return Transform(datasets[dataset_name](), lambda x: Event(
        src=enumerator(int(x.src)),
        dest=enumerator(int(x.dest)),
...
def perform_embedding(cls, alg: str, p: float, q: float, dim: int, dataset_name: str, part_num: int):
    ...
    t = VertexEnumerator()
    dataset = _enumerated_event_stream(dataset_name, t)
