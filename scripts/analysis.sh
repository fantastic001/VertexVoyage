...
for PIPELINE in $PIPELINES; do
    for N_CLUSTERS in 2 3 4 5 6 7 8 9 10; do
        ...
        python -m client analyze_embeddings \
            --single-node-result results/sequential/$PIPELINE/2_process.json \
            --multi-node-result results/sequential/$PIPELINE/2_process.json \
            --clusters $N_CLUSTERS \
            --output $RESULTS_FOLDER/$PIPELINE/$N_CLUSTERS/analysis_dim.csv
        python -m client analyze_embeddings \
            --single-node-result results/sequential/$PIPELINE/3_process.json \
            --multi-node-result results/sequential/$PIPELINE/3_process.json \
            --clusters $N_CLUSTERS \
