...
\begin{itemize}
    ...
    gde je $ f $ deo serijskog dela algoritma u odnosu na ukupno vreme izvršavanja.
    \item Neka je $T_{\text{part}}$ vreme particionisanja, a $T_{\text{embed}}(i)$ vreme ugrađivanja na particiji $i$. Tada je ukupno vreme izvršavanja paralelnog algoritma:
...
\begin{frame}
\begin{itemize}
    \item Iz ovih izraza možemo izvesti formulu za ubrzanje:
    $$ S = \frac{\sum_{i=1}^{K} T_{\text{embed}}(i)}{T_{\text{part}} + \max_{i=1}^{p} T_{\text{embed}}(i)} $$
    \item Ova formula pokazuje kako vreme particionisanja i vreme ugrađivanja na najviše opterećenoj particiji utiču na ukupno ubrzanje paralelnog algoritma.
\end{itemize}
\end{frame}
